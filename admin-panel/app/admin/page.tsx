...
export default function AdminDashboard(){
  ...
  const Dashboard=()=>{
    ...
    return <div style={{display:"flex",flexDirection:"column",gap:20}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:16}}>
        {[
          {label:"Total Revenue",value:`₹${rev.toLocaleString()}`,sub:`₹${weekRev.toLocaleString()} this week`,icon:IndianRupee,color:"#22c55e",trend:"+12%"},
          {label:"Total Orders",value:orders.length,sub:`${orders.filter(o=>o.status==="pending").length} pending`,icon:ShoppingBag,color:"#3b82f6",trend:"+8%"},
          {label:"Today's Orders",value:todayOrders.length,sub:`${todayOrders.filter(o=>o.status==="delivered").length} delivered`,icon:Zap,color:"#a855f7",trend:null},
          {label:"Low Stock Items",value:lowStock.length,sub:"need restocking",icon:AlertCircle,color:"#f59e0b",trend:null},
          ...
          <div key={label} style={{background:"#111827",border:"1px solid rgba(255,255,255,0.07)",borderRadius:16,padding:"20px 22px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
              ...
              {trend&&<span style={{fontSize:11,color:color,background:`${color}18`,padding:"2px 7px",borderRadius:20,fontWeight:600,display:"flex",alignItems:"center",gap:3}}>
                    ...
                    <tr key={p._id} style={{borderBottom:"1px solid rgba(255,255,255,0.04)",transition:"background 0.1s"}}
                      ...
                      <td style={{padding:"12px 16px"}}>
                        <button onClick={()=>toggleProductActive(p)}
                          style={{padding:"4px 10px",borderRadius:20,fontSize:11,cursor:"pointer",border:"none",fontWeight:600,
  ...
  // ── Customers Tab ─────────────────────────────────────────────
  const CustomersTab=()=>{
    const unique=[...new Map(orders.map((o:any)=>[o.customerEmail,o])).values()];
    return(
      <div style={{background:"#111827",border:"1px solid rgba(255,255,255,0.07)",borderRadius:16,overflow:"hidden"}}>
        {unique.length===0
            ...
            {loading
              ...
              :filteredUsers.map((u:any)=>(
                <tr key={u._id} style={{borderBottom:"1px solid rgba(255,255,255,0.04)",transition:"background 0.1s"}}
                  ...
                  <td style={{padding:"13px 16px"}}>
                    <div style={{display:"flex",gap:6}}>
                      {isSuperAdmin&&<button onClick={()=>{setEditUser(u);setEditUserForm({name:u.name,phone:u.phone||"",role:u.role,isActive:u.isActive});}}
                        style={{background:"rgba(59,130,246,0.12)",border:"none",borderRadius:7,padding:"5px 8px",cursor:"pointer",color:"#60a5fa"}}>
                        <Edit2 size={13}/>
                      </button>}
  ...
  const SelectField=({label,value,onChange,options}:any)=>(
    <div style={{marginBottom:14}}>
      ...
      <select value={value} onChange={e=>onChange(e.target.value)}
        ...
        {options.map((o:any)=><option key={o.value||o} value={o.value||o} style={{background:"#111827"}}>{o.label||o}</option>)}
      </select>
    </div>
  );
  ...
  return(
    <div style={{display:"flex",height:"100vh",background:"#0a0d14",fontFamily:"'DM Sans',system-ui,sans-serif",color:"#e2e8f0",overflow:"hidden"}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
              {(["confirmed","preparing","assigned","picked_up","out_for_delivery","delivered","cancelled"] as OrderStatus[]).filter(s=>s!==selectedOrder.status).map(s=>(
                <button key={s} onClick={async()=>{
                  ...
                  style={{padding:"7px 13px",borderRadius:9,fontSize:12,cursor:"pointer",border:"none",fontWeight:500,
      ...
      {showAddUser&&<Modal title="Add New User" onClose={()=>setShowAddUser(false)}>
        <div style={{background:"rgba(168,85,247,0.08)",border:"1px solid rgba(168,85,247,0.2)",borderRadius:10,padding:"10px 14px",marginBottom:16,display:"flex",gap:8,alignItems:"center"}}>
          <ShieldCheck size={14} color="#a855f7"/><span style={{fontSize:12,color:"#c084fc"}}>Super Admin action — creates a real account in Convex DB.</span>
        </div>
        ...
        <FormField label="Phone" value={newUser.phone} onChange={(v:string)=>setNewUser(u=>({...u,phone:v}))} placeholder="9876543210"/>
