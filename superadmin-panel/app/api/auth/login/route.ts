...
export async function POST(req: NextRequest) {
  try {
    ...
    if (email && password && !otp) {
      ...
      return NextResponse.json({ success: false, error: 'Invalid superadmin credentials or inactive' }, { status: 401 })
    }
    ...
    if (email && otp) {
      ...
      if (user.role !== 'superadmin') {
         return NextResponse.json({ success: false, error: 'Access denied. Superadmin role required.' }, { status: 403 })
      }
