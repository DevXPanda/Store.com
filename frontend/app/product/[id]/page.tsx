...
import { ArrowLeft, Star, ShoppingCart, Heart, Leaf, Check, Plus, Minus, Share2, Truck, Shield, RotateCcw } from 'lucide-react'
...
export default function ProductPage() {
  ...
  const handleAddToCart = () => {
    try {
      ...
      const newCount = cart.reduce((s: number, i: any) => s + i.qty, 0)
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#FEFAE0', paddingTop:80 }}>
      ...
      <div style={{ maxWidth:1100, margin:'0 auto', padding:'0 20px 48px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:40, alignItems:'start' }}>
        {/* Left — Image */}
        <div>
          <div style={{ borderRadius:24, overflow:'hidden', background:'white', border:'1px solid #e5e7eb', position:'relative', aspectRatio:'1' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10, marginTop:16 }}>
            {[
              ...
              { icon:Shield, label:'Quality Guarantee', sub:'100% fresh' },
        ...
        <div>
          ...
          <div style={{ background:'#f0fdf4', borderRadius:16, padding:'16px 20px', marginBottom:20, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
            ...
            <div style={{ textAlign:'right' }}>
              <div style={{ fontSize:14, fontWeight:700, color:'#16a34a' }}>Save ₹{product.originalPrice - product.price}</div>
          ...
          {activeTab === 'nutrition' && (
            <div style={{ fontSize:14, color:'#374151' }}>
              <div style={{ background:'#f0fdf4', borderRadius:14, padding:'16px', marginBottom:12 }}>
                ...
                <p style={{ lineHeight:1.7 }}>{product.nutritionHighlight || `${product.name} is packed with essential nutrients and is a great addition to a healthy diet. Rich in vitamins and minerals, it supports overall well-being.`}</p>
              </div>
                  ...
                  {[5,4,3,2,1].map(star => {
                    ...
                    return (
                      <div key={star} style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4 }}>
                        ...
                        <div style={{ flex:1, height:6, background:'#e5e7eb', borderRadius:20 }}>
                          <div style={{ height:'100%', background:'#f59e0b', borderRadius:20, width:`${pct}%` }} />
                        </div>
                        <span style={{ fontSize:11, color:'#9ca3af', width:24 }}>{pct}%</span>
                      </div>
                    )
                  })}
                </div>
              </div>
              <p style={{ fontSize:13, color:'#9ca3af', textAlign:'center' }}>Verified purchases from VegFru customers</p>
            </div>
          )}
        </div>
      </div>
